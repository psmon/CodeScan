/**
 * Menu definition — each item maps to a hash route (#id) and a dynamic
 * import target under js/views/*.js.
 *
 * CodeScan variant — trimmed from the AgentZeroLite reference set:
 *   - drop on-device-model views (agent-models, models, principles,
 *     webdev-plugin, actor-world) — those belong to the sibling
 *     AgentZeroLite project, not CodeScan.
 *   - drop missions / design until CodeScan ships matching content
 *     (harness/missions/, Docs/design/*.pen).
 */
export const MENU = [
  { id: 'dashboard',     label: 'Dashboard',       icon: 'grid',      section: 'main', view: 'dashboard' },
  { id: 'workflow',      label: 'Workflow',        icon: 'branch',    section: 'main', view: 'workflow' },
  { id: 'role',          label: 'Roles',           icon: 'users',     section: 'main', view: 'role' },
  { id: 'skill',         label: 'Skills',          icon: 'zap',       section: 'main', view: 'skill' },
  { id: 'knowledge',     label: 'Knowledge',       icon: 'book',      section: 'main', view: 'knowledge' },
  { id: 'usage-log',     label: 'Activity Log',    icon: 'scroll',    section: 'main', view: 'usage-log' },

  { id: 'product-intro', label: 'Product Intro',   icon: 'box',       section: 'secondary', view: 'product-intro' },
];

/**
 * Minimal inline SVG icon map (stroke-style, lucide-inspired).
 * Keep only icons used by the menu above; drop AZL-specific (cpu/network/
 * orbit/puzzle/sparkles/pieChart/tag/layers/checkSquare/...) to keep the
 * bundle slim. Re-add as needed when new views land.
 */
export const ICONS = {
  ...
  branch:  '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="6" y1="3" x2="6" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/></svg>',
  ...
  scroll:  '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h3"/><path d="M16 3h3a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-3"/><path d="M4 21h16"/><path d="M8 3v18"/><path d="M16 3v18"/></svg>',
  ...
  chevLeft: '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>',
  ...
  pieChart: '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"/><path d="M22 12A10 10 0 0 0 12 2v10z"/></svg>',
