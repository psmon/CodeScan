...
Claude나 Codex를 자유롭게 쓰지 못하는 상황이 분명히 존재합니다 — 폐쇄망, 보안이 강한 격리된 공간, 그리고 최근 자주 발생하는 핵심 AI 벤더의 장애. **그러한 상황에서도 CodeScan의 코드 분석 활동은 멈추지 않을 수 있습니다.** TUI ChatMode는 Gemma 4 모델을 완전히 온디바이스 + CPU 전용으로 구동하면서 CodeScan의 SQLite 인덱스에 JSON 툴콜 루프를 돌립니다. *CPU가 작동한다면, 당신의 코드베이스에 계속 질문할 수 있습니다.*

![CodeScan TUI ChatMode](Home/img/TUI-ChatMode.png)

- **오프라인 설계** — 네트워크 호출 없음. 모델 GGUF는 `~/.codescan/models/`에 두고, 에이전트 루프는 로컬 인덱스와 로컬 파일시스템에만 접근합니다.
- **CPU 전용, 멀티 OS** — Native AOT 그대로 유지. GPU / 드라이버 / 벤더 SDK 모두 불필요.
- **툴 사용 에이전트** — Gemma는 매 턴 GBNF 제약 JSON을 발행합니다 (`db_search` / `read_file` / `grep_file` / `list_projects` / `project_info` / `graph_query` / `done`). 모델이 코드베이스를 추측하지 않고 실제로 들여다봅니다.
- **포렌식 세션 로그** — 모든 채팅 세션이 `~/.codescan/logs/chat-YYYYMMDD_HHmmss.log`에 raw 모델 출력까지 기록돼 추후 분석 가능합니다.
