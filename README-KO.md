...
> **Linux에서 왜 `sudo`인가?** 시스템 패키지로 깐 Node는 npm 글로벌 prefix(`/usr/local/lib/node_modules/`)가 root 소유라 일반 사용자의 `npm install -g`는 `EACCES`로 실패합니다. sudo를 쓰지 않으려면 **nvm/fnm**으로 Node를 사용자 홈에 두거나, 아래의 [직접 인스톨러](#direct-installer-fallback)를 쓰세요 (npm·sudo 모두 불필요).
