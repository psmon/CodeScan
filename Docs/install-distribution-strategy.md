...
| Linux 채널 | npm 글로벌 래퍼 (`@webnori/codescan-cli` — 짧은 `codescan-cli`는 제3자 squat) |
...
> 짧은 이름 `codescan-cli`는 2026-03경 무관한 npm 사용자(`o-town`)가 v1.0.0으로 선점했으므로 우리는 사용할 수 없다. 조직 스코프 `@webnori`(npm org) 하위에 publish하여 점유한다. publish 명령은 `npm publish --access public`이어야 한다 (스코프 패키지 기본값은 private).

### 검토 결과 및 결정

| 항목 | 결정 |
|------|------|
| 패키지명 | `@webnori/codescan-cli` (스코프 패키지 — 짧은 `codescan-cli`는 제3자 squat으로 확보 불가) |
...
npm uninstall -g @webnori/codescan-cli     # Linux
...
- 패키지 매니저별 표준 흐름 (`winget install --version`, `npm install -g @webnori/codescan-cli@x.y.z`, `brew install codescan@x.y.z`).
...
5. **npm 래퍼 publish** — `@webnori/codescan-cli` (postinstall 다운로드 방식, `npm publish --access public`)
